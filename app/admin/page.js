...
export default function AdminPage() {
  ...
  const [instructions, setInstructions] = useState([{ ge: '', en: '', image_file: null, image_preview: null }]);
  ...
  const handleSubmit = async () => {
    if (!form.slug) { setMessage('❌ Slug (URL) სავალდებულოა!'); return; }
    setLoading(true);
    setMessage('');
    try {
      ...
      setInstructions([{ ge: '', en: '', image_file: null, image_preview: null }]);
      ...
      <div className="max-w-4xl mx-auto">
        <div className="flex items-center gap-3 mb-10">
          ...
          <div>
            ...
            <p className="text-sm text-[#7a6452]">ახალი რეცეპტის დამატება — ფოტოები Cloudinary-ზე ინახება</p>
          </div>
        </div>
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white rounded-2xl p-6 shadow-sm">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {[
                ['slug','Slug (URL)','khinkali'],
                ['prep_time','დრო (წუთი)','90'],
                ['calories','კალორია (kcal)','350'],
                ['country_ge','ქვეყანა (ქართ.)','საქართველო'],
                ['country_en','ქვეყანა (ინგლ.)','Georgia'],
                ['country_code','ქვეყნის კოდი','GE'],
              ].map(([key, label, ph]) => (
                <div key={key}>
          ...
          {/* Main Image */}
          <div className="bg-white rounded-2xl p-6 shadow-sm">
            <h2 className="text-xl font-semibold text-[#1a120a] mb-1">მთავარი სურათი</h2>
            <p className="text-xs text-[#7a6452] mb-4">☁️ Cloudinary-ზე ატვირთვა — კომპიუტერიდან წაშლის შემდეგაც საიტზე დარჩება</p>
            <div className="flex gap-6 items-start">
              <label className="flex-1 flex flex-col items-center justify-center h-40 border-2 border-dashed border-[#f5e6cc] rounded-xl cursor-pointer hover:border-[#c1704a] transition-colors bg-[#fdf6ec]">
                ...
                <span className="text-xs text-[#7a6452]/60 mt-1">JPG, PNG, WebP</span>
          ...
          {/* Instructions */}
          <div className="bg-white rounded-2xl p-6 shadow-sm">
            <h2 className="text-xl font-semibold text-[#1a120a] mb-1">მომზადების ნაბიჯები</h2>
            <p className="text-xs text-[#7a6452] mb-5">თითოეული ნაბიჯისთვის შეგიძლია ფოტო ატვირთო — Cloudinary-ზე შეინახება</p>
            <div className="space-y-6">
                    ...
                    <div className="flex gap-3 items-center">
                      ...
                      {ins.image_preview && (
                        <div className="w-16 h-16 rounded-lg overflow-hidden shrink-0">
                          <img src={ins.image_preview} alt="step" className="w-full h-full object-cover" />
                        </div>
                      )}
                      {ins.image_preview && (
                        <button onClick={() => { const u=[...instructions]; u[i].image_file=null; u[i].image_preview=null; setInstructions(u); }} className="text-rose-400 hover:text-rose-600 text-xs">წაშლა</button>
                      )}
                    </div>
                  </div>
                </div>
              ))}
              <button onClick={() => setInstructions([...instructions, {ge:'',en:'',image_file:null,image_preview:null}])} className="flex items-center gap-2 text-sm text-[#2d5a27] mt-2"><Plus size={16} /> ნაბიჯის დამატება</button>
            </div>
          </div>
          ...
          <button onClick={handleSubmit} disabled={loading} className="w-full flex items-center justify-center gap-2 py-4 bg-[#c1704a] hover:bg-[#9a4f2a] disabled:opacity-50 text-white rounded-xl font-medium transition-all">
            <Save size={18} />
            {loading ? '☁️ Cloudinary-ზე იტვირთება...' : 'რეცეპტის შენახვა'}
          </button>
        </div>
      </div>
    </div>
